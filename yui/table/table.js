	...
	M.report_teacherreport.table = {
			init : function(){
				...
				Y.all('.main').on('click',function(e){
					 e.preventDefault();
					 if(Y.one('.'+e.currentTarget.get('id')).hasClass('disabeld')){
						var numofRows = Y.all('.'+e.currentTarget.get('id'))._nodes.length+1;
